...
-- **EXT - Ableton Link** : Use the BPM values from the Ableton DAW. Values are from 20 to 240. Manually do the midi mapping and set the BPM scale in Ableton the max value to 240. 

-- **EXT - Reason** : Use the BPM values from the Reason DAW (or any other you use). Default values are from 1 to 244. Manually do the midi mapping. If you use any other DAW, modify in the template script the MIDI Control Change message of the **BPMRS** control to set the scale to your DAW as needed. The default scale for Reason is setup from 0 to 30. To change the name to your DAW, locate in the script the INTEXT and DAW controls the variable **daw_ext** and change its value. 
...
### Note (Pitch) 
Select the value of the note on each step. It can be changed at any time and its value is stored automatically in the corresponding snapshot. The steps available depend on the number of steps selected in the snapshot. Press th **Note** button to display the  note values. Press the **zoom** button to expand the faders so you can more easily input a note value. 
...
- DAW Host Mapping:  MIDI. Manually  do the midi mapping in the PLAY, STOP and BPM controls between the template and your DAW.
